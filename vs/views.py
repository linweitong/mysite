        ...
        if int(pk) == 0:
            ...
            if places.count() > 0:
                place = places[0]
                    ...
                    place_search=str.format('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=%s,%s&rankby=distance&types=bar|night_club|cafe&sensor=false&key=%s' %(latitude, longitude,settings.GOOGLE_PLACE_API_KEY))
                    response = urlopen(place_search).read()
                    results =json.loads(response)
                    if results["status"] == 'OK':
